package com.metoo.nspm.core.config.utils.poi;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.metoo.nspm.core.config.utils.MyStringUtils;
...
public class ExcelUtil {
    ...
    public static <T> List<MobileWhiteList> readMultipartFile(MultipartFile  file, Class<T> clazz) throws Exception {
        JSONArray array = readMultipartFile(file);
        List<MobileWhiteList> list = new ArrayList<>();
        for (int i = 0; i < array.size(); i++) {
            MobileWhiteList mobileWhiteList = JSON.toJavaObject(array.getJSONObject(i), MobileWhiteList.class);
            list.add(mobileWhiteList);
        }
        return list;
    }
